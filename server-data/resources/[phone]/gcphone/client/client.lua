...
function showFixePhoneHelper (coords)
  for number, data in pairs(FixePhone) do
    ...
    if dist <= 2.0 then
      ...
      AddTextComponentString("~g~" .. data.name .. ' ~o~' .. number .. '~n~~INPUT_PICKUP~~w~ 使用')
  ...
  while true do 
    ...
    for i, _ in pairs(PhoneInCall) do 
        ...
        if (dist <= soundDistanceMax) then
          ...
          if (dist <= 1.5) then 
            SetTextComponentFormat("STRING")
            AddTextComponentString("~INPUT_PICKUP~ 接電話")
    ...
    elseif inRangeToActivePhone == true then
      ...
      if (mod == 15) then
        mod = 0
        SetSoundVolumeJS('ring2.ogg', 0.1 + (inRangedist - soundDistanceMax) / -soundDistanceMax * 0.8 )
      end
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 then
    ...
    if ShowNumberNotification == true then
      text = '~o~來自 ~y~'.. message.transmitter.. " ~s~的新訊息"
      for _,contact in pairs(contacts) do
        if contact.number == message.transmitter then
          text = '~o~來自 ~g~'.. contact.display.. " ~s~的新訊息"
          break
        end
      end
    end
