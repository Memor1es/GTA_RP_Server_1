...
local DescriptionMsg = "~y~請使用滑鼠選擇，W.A.S.D 以及 V 調整視角"
...
local submenuFaceShape = _menuPool:AddSubMenu(freemodePedMenu, "微整形", "擦擦玻尿酸，吃吃肉毒桿菌", Config.MenuX, Config.MenuY,nil,nil,nil,Config.MenuColor.r,Config.MenuColor.g,Config.MenuColor.b,Config.MenuColor.a)
local submenuAppearance = _menuPool:AddSubMenu(freemodePedMenu, "化妝室", "幫自己換個造型搭配新面孔", Config.MenuX, Config.MenuY,nil,nil,nil,Config.MenuColor.r,Config.MenuColor.g,Config.MenuColor.b,Config.MenuColor.a)
--local submenuClothes = _menuPool:AddSubMenu(freemodePedMenu, "Clothes", "~b~Character Clothing Options", Config.MenuX, Config.MenuY,nil,nil,nil,Config.MenuColor.r,Config.MenuColor.g,Config.MenuColor.b,Config.MenuColor.a)
...
function AddMenuFaceShape(submenu)
	...
	local ListItem = {
		...
		local GridItem =  NativeUI.CreateListItem("鼻子", ListItem, 1, DescriptionMsg)
		local GridPanel = NativeUI.CreateGridPanel("靠上", "嬌小", "寬大", "靠下")
		...
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
			...
			TriggerEvent('skinchanger:change', 'nose_1', panelvalue.X)
			TriggerEvent('skinchanger:change', 'nose_2', panelvalue.Y)
			--print(panelvalue.X.."---"..panelvalue.Y)
		end
		--Nose Profile Crooked Curved Short Long
		local GridItem =  NativeUI.CreateListItem("鼻骨", ListItem, 1)
		local GridPanel = NativeUI.CreateGridPanel("彎曲", "較長", "較短", "平整")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
			...
			TriggerEvent('skinchanger:change', 'nose_3', panelvalue.X)
			TriggerEvent('skinchanger:change', 'nose_4', panelvalue.Y)
		end
		--Nose Tip Tip Up Tip Down Broken Left Broken Right
		local GridItem =  NativeUI.CreateListItem("鼻型", ListItem, 1)
		local GridPanel = NativeUI.CreateGridPanel("向左", "向上", "向下", "向右")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
			...
			TriggerEvent('skinchanger:change', 'nose_5', panelvalue.X)
			TriggerEvent('skinchanger:change', 'nose_6', panelvalue.Y)
		end
		
		--Eye Brows-- up down in out
		local GridItem =  NativeUI.CreateListItem("眉骨", ListItem, 1)
		local GridPanel = NativeUI.CreateGridPanel("向上", "向內", "向外", "向下")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
			...
			TriggerEvent('skinchanger:change', 'eyebrows_5', panelvalue.X)
			TriggerEvent('skinchanger:change', 'eyebrows_6', panelvalue.Y)
		end
		
		--Cheeks Bones Up Down In Out
		local GridItem =  NativeUI.CreateListItem("顴骨", ListItem, 1)
		local GridPanel = NativeUI.CreateGridPanel("向內", "向上", "向下", "向外")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
			...
			TriggerEvent('skinchanger:change', 'cheeks_1', panelvalue.X)
			TriggerEvent('skinchanger:change', 'cheeks_2', panelvalue.Y)
		end
		
		--Cheeks Gaunt Puffed
		local GridItem =  NativeUI.CreateListItem("臉頰", ListItem, 1)
		local GridPanel = NativeUI.CreateHorizontalGridPanel("飽滿", "凹陷")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
			SetPedFaceFeature(GetPlayerPed(-1), 10, panelvalue.X);						--"Cheeks Width",             // 10
			Character['cheeks_3'] = panelvalue.X
			TriggerEvent('skinchanger:change', 'cheeks_3', panelvalue.X)
		end
		
		--Eyes-- Squint Wide
		local GridItem =  NativeUI.CreateListItem("眼型", ListItem, 1)
		local GridPanel = NativeUI.CreateHorizontalGridPanel("睜大眼", "瞇瞇眼")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
			SetPedFaceFeature(GetPlayerPed(-1), 11, panelvalue.X);						--"Eyes Opening",             // 11
			Character['eye_open'] = panelvalue.X
			TriggerEvent('skinchanger:change', 'eye_open', panelvalue.X)
		end

		--Lips Thin Fat
		local GridItem =  NativeUI.CreateListItem("唇形", ListItem, 1)
		local GridPanel = NativeUI.CreateHorizontalGridPanel("豐厚", "細薄")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
			SetPedFaceFeature(GetPlayerPed(-1), 12, panelvalue.X);						--"Lips Thickness",           // 12
			Character['lips_thick'] = panelvalue.X
			TriggerEvent('skinchanger:change', 'lips_thick', panelvalue.X)
		end
		--Jaw Round Square Narrow Wide
		local GridItem =  NativeUI.CreateListItem("顎骨", ListItem, 1)
		local GridPanel = NativeUI.CreateGridPanel("較窄", "圓滑", "方正", "較寬")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
			...
			TriggerEvent('skinchanger:change', 'jaw_1', panelvalue.X)
			TriggerEvent('skinchanger:change', 'jaw_2', panelvalue.Y)
		end
		--Chin Profile Up Down In Out
		local GridItem =  NativeUI.CreateListItem("下巴", ListItem, 1)
		local GridPanel = NativeUI.CreateGridPanel("向上", "向內", "向外", "向下")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
			...
			TriggerEvent('skinchanger:change', 'chin_height', panelvalue.X)
			TriggerEvent('skinchanger:change', 'chin_lenght', panelvalue.Y)
		end
		-- Chin Shape Rounded Bum Square Pointed
		local GridItem =  NativeUI.CreateListItem("下巴形狀", ListItem, 1)
		local GridPanel = NativeUI.CreateGridPanel("圓", "尖", "方", "凹")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
			...
			TriggerEvent('skinchanger:change', 'chin_width', panelvalue.X)
			TriggerEvent('skinchanger:change', 'chin_hole', panelvalue.Y)
		end
		--Neck Thickness Thin Fat
		local GridItem =  NativeUI.CreateListItem("頸部", ListItem, 1)
		local GridPanel = NativeUI.CreateHorizontalGridPanel("瘦", "胖")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
			SetPedFaceFeature(GetPlayerPed(-1), 19, panelvalue.X);						--"Neck Thickness"            // 19
			Character['neck_thick'] = panelvalue.X
			TriggerEvent('skinchanger:change', 'neck_thick', panelvalue.X)
		end
...
function AddMenuAppearance(submenu)
	...
	local Description = DescriptionMsg	
	--local maxTextures = GetNumberOfPedTextureVariations(GetPlayerPed(-1), i, currentVariationIndex);
	-- new select
	local maxDrawables = 79
	maxDrawables = maxDrawables - 1
	amount = {}
	for i = 1, maxDrawables do
		amount[i] = ""..i.."/"..maxDrawables
	end
	local Select = NativeUI.CreateListItem("髮型", amount , 1, Description)
	-- new select end
	--local Select = NativeUI.CreateListItem("Haircut", maxhair , 1, Description )
	local BaseColor = NativeUI.CreateColourPanel("顏色", ColoursPanel.HairCut)
	...
	Select.OnListChanged = function(ParentMenu, SelectedItem, Index)
		...
		TriggerEvent('skinchanger:change', 'hair_color_1', color-1)
	end
...
-- Eyebrows + opacity Color
	...
	local Description = DescriptionMsg
	-- new select
	local maxDrawables = GetNumHeadOverlayValues(2)
	maxDrawables = maxDrawables - 1
	amount = {}
	for i = 1, maxDrawables do
		amount[i] = ""..i.."/"..maxDrawables
	end
	local Select = NativeUI.CreateListItem("眉毛", amount , 1, Description)
	-- new select end
	--local Select = NativeUI.CreateListItem("Eye Brows", eyebrow , 1, Description )
	local BaseColor = NativeUI.CreateColourPanel("顏色", ColoursPanel.HairCut)
	local PercentageItem = NativeUI.CreatePercentagePanel("0%", "透明度", "100%")
	...
	Select.OnListChanged = function(ParentMenu, SelectedItem, Index)
		...
		TriggerEvent('skinchanger:change', 'eyebrows_3', color-1)
	end
...
--Facial Hair : color Opa
	...
	local Description = DescriptionMsg
	-- new select
	local maxDrawables = GetNumHeadOverlayValues(1)
	maxDrawables = maxDrawables - 1
	amount = {}
	for i = 1, maxDrawables do
		amount[i] = ""..i.."/"..maxDrawables
	end
	local Select = NativeUI.CreateListItem("鬍鬚", amount , 1, Description)
	-- new select end
	...
	Select.OnListChanged = function(ParentMenu, SelectedItem, Index)
		...
		TriggerEvent('skinchanger:change', 'beard_3', color-1)
	end
	...
	local Description = DescriptionMsg
	-- new select
	local maxDrawables = GetNumHeadOverlayValues(4)
	maxDrawables = maxDrawables - 1
	amount = {}
	for i = 1, maxDrawables do
		amount[i] = ""..i.."/"..maxDrawables
	end
	local Select = NativeUI.CreateListItem("眼妝", amount , 1, Description)
	-- new select end
	...
	Select.OnListChanged = function(ParentMenu, SelectedItem, Index)
		...
		TriggerEvent('skinchanger:change', 'makeup_3', color-1)
	end
	...
	local Description = "更換隱形眼鏡"
	-- new select
	local maxDrawables = 33
	maxDrawables = maxDrawables - 1
	amount = {}
	for i = 1, maxDrawables do
		amount[i] = ""..i.."/"..maxDrawables
	end
	local Select = NativeUI.CreateListItem("隱形眼鏡", amount , 1, Description)
	-- new select end
	--local Select = NativeUI.CreateListItem("隱形眼鏡", eyeColorNames , 1, Description )
	submenu.SubMenu:AddItem(Select)
	Select.OnListChanged = function(ParentMenu, SelectedItem, Index)
	...
	local Description = DescriptionMsg
	-- new select
	local maxDrawables = GetNumHeadOverlayValues(8)
	maxDrawables = maxDrawables - 1
	amount = {}
	for i = 1, maxDrawables do
		amount[i] = ""..i.."/"..maxDrawables
	end
	local Select = NativeUI.CreateListItem("口紅", amount , 1, Description)
	-- new select end
	...
	Select.OnListChanged = function(ParentMenu, SelectedItem, Index)
		...
		TriggerEvent('skinchanger:change', 'lipstick_3', color-1)
	end
	...
	local Description = DescriptionMsg
	-- new select
	local maxDrawables = GetNumHeadOverlayValues(5)
	maxDrawables = maxDrawables - 1
	amount = {}
	for i = 1, maxDrawables do
		amount[i] = ""..i.."/"..maxDrawables
	end
	local Select = NativeUI.CreateListItem("腮紅", amount , 1, Description)
	-- new select end
	...
	Select.OnListChanged = function(ParentMenu, SelectedItem, Index)
		...
		TriggerEvent('skinchanger:change', 'blush_3', color-1)
	end
	...
	local Description = DescriptionMsg
	-- new select
	local maxDrawables = GetNumHeadOverlayValues(0)
	maxDrawables = maxDrawables - 1
	amount = {}
	for i = 1, maxDrawables do
		amount[i] = ""..i.."/"..maxDrawables
	end
	local Select = NativeUI.CreateListItem("Blemishes", amount , 1, Description)
	-- new select end
	...
	local Description = DescriptionMsg
	-- new select
	local maxDrawables = GetNumHeadOverlayValues(3)
	maxDrawables = maxDrawables - 1
	amount = {}
	for i = 1, maxDrawables do
		amount[i] = ""..i.."/"..maxDrawables
	end
	local Select = NativeUI.CreateListItem("老化", amount , 1, Description)
	-- new select end
	...
	local Description = DescriptionMsg
	-- new select
	local maxDrawables = GetNumHeadOverlayValues(6)
	maxDrawables = maxDrawables - 1
	amount = {}
	for i = 1, maxDrawables do
		amount[i] = ""..i.."/"..maxDrawables
	end
	local Select = NativeUI.CreateListItem("胎記", amount , 1, Description)
	-- new select end
	...
	local Description = DescriptionMsg
	-- new select
	local maxDrawables = GetNumHeadOverlayValues(9)
	maxDrawables = maxDrawables - 1
	amount = {}
	for i = 1, maxDrawables do
		amount[i] = ""..i.."/"..maxDrawables
	end
	local Select = NativeUI.CreateListItem("雀斑", amount , 1, Description)
	-- new select end
	...
	local Description = DescriptionMsg
	-- new select
	local maxDrawables = GetNumHeadOverlayValues(7)
	maxDrawables = maxDrawables - 1
	amount = {}
	for i = 1, maxDrawables do
		amount[i] = ""..i.."/"..maxDrawables
	end
	local Select = NativeUI.CreateListItem("曬傷", amount , 1, Description)
	-- new select end
