...
AddEventHandler('rs_crafting:CraftingSuccess', function(CraftItem)
    ...
    TriggerClientEvent('esx:showNotification', src, "你製作了 ~b~"..item.label.."~w~!")
end)
-- Fail server event
...
AddEventHandler('rs_crafting:CraftingFailed', function(CraftItem)
    ...
    TriggerClientEvent('esx:showNotification', src, "~r~製作 ~b~"..item.label.."~r~失敗")
end)
...
ESX.RegisterServerCallback('rs_crafting:HasTheItems', function(source, cb, CraftItem,kind)
    local xPlayer = ESX.GetPlayerFromId(source)
    local item = Crafting.Items[CraftItem]
    if kind == 2 then
        item = Crafting.MetalItems[CraftItem]
    elseif kind == 3 then
        item = Crafting.ChemistryItems[CraftItem]
    else
        item = Crafting.NormalItems[CraftItem]
    end
    for itemname, v in pairs(item.needs) do
