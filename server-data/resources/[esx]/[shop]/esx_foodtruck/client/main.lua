			...
			elseif data.current.value == 'gears' then
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'foodtruck_gears', {
						...
						if data.current.value ~= 'clean' then
							...
							--local obj, dist = ESX.Game.GetClosestObject(nil,{GetHashKey('prop_bbq_5'), GetHashKey('prop_table_para_comb_02'), GetHashKey('prop_table_03_chr')})
							--[[
							...
							local obj, dist = ESX.Game.GetClosestEntity(ESX.Game.GetObjects(), false, nil, {[GetHashKey('prop_bbq_5')]=true, [GetHashKey('prop_table_para_comb_02')]=true, [GetHashKey('prop_table_03_chr')]=true})
							if dist < 3.0 then
								DeleteEntity(obj)
							else
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsControlJustReleased(0, Keys["F7"]) and PlayerData.job ~= nil and PlayerData.job.name == 'foodtruck' then
