	...
	while isFueling do
		...
		if not pumpObject then
			if GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * 100 >= 0 then
				...
				ammo = math.floor(fuelToAdd * 100)
				TriggerServerEvent('fuel:removeCanammo',ammo)
			else
				isFueling = false
			end
		else
		...
		if currentFuel > GetVehicleHandlingFloat(vehicle,"CHandlingData","fPetrolTankVolume") then
			currentFuel = GetVehicleHandlingFloat(vehicle,"CHandlingData","fPetrolTankVolume")
			isFueling = false
		end
...
AddEventHandler('fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			DrawText3Ds(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + 0.5, Round(currentFuel * 100.0 / GetVehicleHandlingFloat(vehicle,"CHandlingData","fPetrolTankVolume"), 1) .. "%")
		else
			DrawText3Ds(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + 0.5, Config.Strings.CancelFuelingJerryCan .. "\n汽油桶: ~g~" .. Round(GetAmmoInPedWeapon(ped, 883325847) / 4500 * 100, 1) .. "% | 車輛: " .. Round(currentFuel * 100.0 / GetVehicleHandlingFloat(vehicle,"CHandlingData","fPetrolTankVolume"), 1) .. "%")
		end
			...
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
				...
				if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped), vehicleCoords) < 2.5 then
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetVehicleFuelLevel(vehicle) < GetVehicleHandlingFloat(vehicle,"CHandlingData","fPetrolTankVolume") and canFuel then
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								...
								TriggerServerEvent('fuel:removeCan')
								TriggerServerEvent('fuel:getCan')
