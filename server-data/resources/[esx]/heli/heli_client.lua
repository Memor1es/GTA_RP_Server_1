...
local toggle_lock_on = 247 -- empty control -- control id to lock onto a vehicle with the camera or unlock from vehicle (with or without camera). Default is INPUT_SPRINT (spacebar)
			...
			if IsHeliHighEnough(heli) then
				...
				if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
					...
					if GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed then
						...
						AddTextComponentString("~r~無法從此座位掛繩索")
