...
AddEventHandler('lkrp_notepad:updateNotes', function(serverNotesPassed)
    --print('serverNotesPassed number : ' .. #serverNotesPassed)
    TestLocalTable = serverNotesPassed
    --print('TestLocalTable number : ' .. #TestLocalTable)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if #TestLocalTable == 0 then
            ...
            local closestNoteDistance = 180.0
            ...
            if distance < 3.0 then
                ...
                DrawText3Ds(TestLocalTable[closestNoteId]["x"],TestLocalTable[closestNoteId]["y"],TestLocalTable[closestNoteId]["z"]-0.4, "~g~E~s~ 閱讀,~g~G~s~ 撕掉")
                ...
                if IsControlJustReleased(0, 47) then
                  TriggerServerEvent("server:destroyNote",TestLocalTable[closestNoteId]["id"])
