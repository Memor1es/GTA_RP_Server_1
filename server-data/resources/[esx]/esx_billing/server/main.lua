...
AddEventHandler('esx_billing:sendBill', function(playerId, sharedAccountName, label, amount)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', sharedAccountName, function(account)
		...
		elseif account == nil then
			...
			if xTarget ~= nil then
				MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount, time) VALUES (@identifier, @sender, @target_type, @target, @label, @amount, @time)',
				{
					...
					TriggerClientEvent('esx:showNotification', xTarget.source, _U('received_invoice') .. "，金額為" .. amount)
				end)
			end
			...
			if xTarget ~= nil then
				MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount, time) VALUES (@identifier, @sender, @target_type, @target, @label, @amount, @time)',
				{
					...
					if sharedAccountName == 'society_police' then
						TriggerClientEvent('esx:showNotification', xTarget.source, '你收到了一張罰單~r~「' .. label .. '」~s~- ~g~$' .. amount)
					elseif sharedAccountName == 'society_ambulance' then
						TriggerClientEvent('esx:showNotification', xTarget.source, '你收到了一張 ~r~醫療帳單~s~ - ~g~$' .. amount)
					else
...
ESX.RegisterServerCallback('esx_billing:payBill', function(source, cb, id)
	...
	MySQL.Async.fetchAll('SELECT * FROM billing WHERE id = @id', {
		...
		local time		 = os.time() - result[1].time 

		local xTarget = ESX.GetPlayerFromIdentifier(sender)
		--print("相差秒數"..time)
		time = math.abs(time)
		if Config.EnableExtraFee then
			if time >= Config.IncreaseTime *60 then
				local new_fee = math.floor(time/(Config.IncreaseTime *60)) * (1 + Config.IncreasePercentage)  --per_time*(1+percentage)
				local extra_fee = new_fee - amount
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('extra_fee', ESX.Math.GroupDigits(extra_fee)))
				amount = new_fee
			end
		end
