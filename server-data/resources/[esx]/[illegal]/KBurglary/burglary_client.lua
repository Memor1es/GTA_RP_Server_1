local Keys = {
  ...
  local textUnlock = "~g~[E]~w~ 進入" -- enter the house
  local insideText = "~g~[E]~w~ 離開" -- exit the door
  local abortConfirm = "你停止了開鎖"
  local searchText = "~g~[E]~w~ 搜索" -- search the spot
  local emptyMessage = "附近沒有東西!" -- if you press E where it is empty
  local emptyMessage3D = "~r~空的" -- if the spot is empty
  local closetText = "~g~[E]~w~ 偷看壁櫥" -- text at closet
  local abortLock = "~g~[E]~w~ 停止開鎖"
  local noLockpickText = "您沒有任何開鎖工具!" -- if you don't have a lockpick and you try to do the burglary
  local carUnlocked = "您已解鎖汽車"
  local youFound = "從" -- when you steal something
  local burglaryDetected = "檢測到盜竊案" -- text 1 cops gets sent
  local sentPhoto = "我們已向您發送了罪犯的照片." -- if you use qalle's camerasystem this will be in the message too
  local item = {'ring', 'goldNecklace', 'laptop', 'bottle', 'weed_pooch', 'samsungS10', 'rolex', 'camera','bread','water','bread','water','bread','water'}
  ...
  TriggerEvent('instance:registerType', 'burglary')
  ...
  AddEventHandler('99kr-burglary:Lockpick', function()
    --GetClockTime()
    if GetClockHours() == 1 or GetClockHours() == 2 or GetClockHours() == 22 or GetClockHours() == 23 or GetClockHours() == 00 or GetClockHours() == 3 or GetClockHours() == 4 then
      ...
      ESX.TriggerServerCallback('99kr-burglary:checkcop', function(cops)
        if cops then
          lockpicking = true
          Citizen.Wait(100)
          lockpicking = false
        else
          ESX.ShowNotification("警察巡邏中")
        end
  ...
  [" 廚房抽屜，你找到 "] = { x = 342.23, y = -1003.29, z = -99.0,  amount = 0},
  [" 電視抽屜，你找到 "] = { x = 338.14, y = -997.69,  z = -99.2,  amount = 0},
  [" 房間抽屜，你找到 "] = { x = 350.91, y = -999.26,  z = -99.2,  amount = 0},
  [" 床頭櫃，你找到 "] = { x = 349.19, y = -994.83,  z = -99.2,  amount = 0},
  [" 書架，你找到 "] = { x = 345.3,  y = -995.76,  z = -99.2,  amount = 0},
  [" 走道桌子，你找到 "] = { x = 346.14, y = -1001.55, z = -99.2,  amount = 0},
  [" 房間抽屜，你找到 "] = { x = 347.23, y = -994.09,  z = -99.2,  amount = 0},
  [" 餐桌上，你找到 "] = { x = 339.23, y = -1003.35, z = -99.2,  amount = 0},
  [" 衣櫃，你找到 "] = { x = 351.24, y = -993.53,  z = -99.2,  amount = 0}
    ...
    Citizen.CreateThread(function()
      ...
      local LockpickAmount = nil
        for i=1, #inventory, 1 do                          
          if inventory[i].name == 'lockpick_house' then
          ...
          if LockpickAmount > 0 then
            ...
            if random <= chancePoliceNoti then 
              ...
              TriggerServerEvent('esx_outlawalert:burglaryInProgress', {
                x = ESX.Math.Round(exitPos.pos.x, 1),
                y = ESX.Math.Round(exitPos.pos.y, 1),
                z = ESX.Math.Round(exitPos.pos.z, 1)
            })
            end
          else 
  ...
  function HouseBreak(house)
    ...
    if rand < 9 then
      TriggerServerEvent('99kr-burglary:Remove', 'lockpick_house', 1)
    end  
    fade()
    ClearPedTasks(playerPed)
    FreezeEntityPosition(playerPed, false)
  ...
  function SpawnResidents(home)
      ...
      for _,resident in pairs(residents) do
        ...
        local ran = math.random() + math.random(1, 6)
        SetPedHearingRange(ped, ran)
        SetPedSeeingRange(ped, ran)
  ...
  function teleport(confMenu)
    ...
    local playerPed = PlayerPedId()
    SetCoords(playerPed, confMenu.pos.x, confMenu.pos.y, confMenu.pos.z )
  ...
  Citizen.CreateThread(function()
    ...
    while true do
        ...
        for k in pairs(gym) do
          ...
          local dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, gym[k].x, gym[k].y, gym[k].z)
          if dist <= 0.5 then
            DrawText3D(plyCoords.x, plyCoords.y, plyCoords.z, "~w~按下 ~r~[H] ~w~ 來使用古董商!", 0.4)
...
Citizen.CreateThread(function()
  ...
  while true do
      --Citizen.Wait(0)
      for k in pairs(coin_shop) do
        local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
        local dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, coin_shop[k].x, coin_shop[k].y, coin_shop[k].z)
        if dist <= 3 then
          DrawMarker(21, coin_shop[k].x, coin_shop[k].y, coin_shop[k].z, 0, 0, 0, 0, 0, 0, 0.301, 0.301, 0.3001, 0, 153, 255, 255, 0, 0, 0, 0)
          sleep = 5
        else
          sleep = 5*1000
        end
...
Citizen.CreateThread(function()
  ...
  while true do
      ...
      for k in pairs(coin_shop) do
        ...
        local dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, coin_shop[k].x, coin_shop[k].y, coin_shop[k].z)
        if dist <= 0.5 then
          DrawText3D(plyCoords.x, plyCoords.y, plyCoords.z, "~w~按下 ~r~[H] ~w~ 來進行交易!", 0.4)
          if IsControlJustPressed(0, Keys['H'])then
            OpenCoinMenu()
          end
          sleep = 5
...
function OpenCoinMenu()
  ...
  ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'coin_exchange_menu',
      {
          title    = '你有帶來我中意的東西嗎?',
          elements = {
              {label = '#1: ', value = 'recipe_IronCircle'},
              {label = '#2: ', value = 'recipe_handcuffs'},
              {label = '#3: ', value = 'recipe_lockpick'},
              {label = '#4: ', value = 'recipe_string'},
              {label = '#5: ', value = 'recipe_bomb'},
              {label = '#6: ', value = 'recipe_rope'},
          }
      },
      function(data, menu)
          if data.current.value == 'recipe_IronCircle' then
              TriggerServerEvent('99kr-burglary:CoinExchange','recipe_IronCircle',Config.exchange_item,15)
          elseif data.current.value == 'recipe_handcuffs' then
              TriggerServerEvent('99kr-burglary:CoinExchange','recipe_handcuffs',Config.exchange_item,15)
          elseif data.current.value == 'recipe_lockpick' then
              TriggerServerEvent('99kr-burglary:CoinExchange','recipe_lockpick',Config.exchange_item,15)
          elseif data.current.value == 'recipe_string' then
              TriggerServerEvent('99kr-burglary:CoinExchange','recipe_string',Config.exchange_item,15)
          elseif data.current.value == 'recipe_bomb' then
              TriggerServerEvent('99kr-burglary:CoinExchange','recipe_bomb',Config.exchange_item,25)
          elseif data.current.value == 'recipe_rope' then
              TriggerServerEvent('99kr-burglary:CoinExchange','recipe_rope',Config.exchange_item,10)
          end
      end,
      function(data, menu)
          menu.close()
