...
RegisterServerEvent('esx_doorlock:PassLock')
AddEventHandler('esx_doorlock:PassLock', function(doorID, state)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if doorID.KeyPass==nil then
		print("there is no key enter this house")
		return
	end

	if xPlayer.getInventoryItem(doorID.KeyPass)==nil then
		print("u dont know the key")
		return
	end

	if xPlayer.getInventoryItem(doorID.KeyPass).count<1 then
		print("u dont have a key to enter")
		return
	end
	-- make each door a table, and clean it when toggled
	DoorInfo[doorID] = {}
...
ESX.RegisterServerCallback('esx_doorlock:getItemAmount', function(source, cb, item)
    local xPlayer = ESX.GetPlayerFromId(source)
