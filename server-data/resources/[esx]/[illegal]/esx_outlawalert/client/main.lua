...
RegisterNetEvent('esx_outlawalert:outlawGunShotNotify')
AddEventHandler('esx_outlawalert:outlawGunShotNotify', function(alert, targetCoords)
	local pos = GetEntityCoords(GetPlayerPed(-1), true)

	if (Vdist(pos.x, pos.y, pos.z, targetCoords.x, targetCoords.y, targetCoords.z) < Config.BlipGunAlertRadius) then
		ESX.ShowNotification(alert)
	end
end)
...
RegisterNetEvent('esx_outlawalert:burglaryInProgress')
AddEventHandler('esx_outlawalert:burglaryInProgress', function(targetCoords)
	if isPlayerWhitelisted then
		--if Config.CarJackingAlert then
			local alpha = 250
			local thiefBlip = AddBlipForRadius(targetCoords.x, targetCoords.y, targetCoords.z, Config.BlipBurglaryTime)
			...
			while alpha ~= 0 do
				Citizen.Wait(Config.BlipBurglaryRadius * 4)
				alpha = alpha - 1
				SetBlipAlpha(thiefBlip, alpha)
...
AddEventHandler('esx_outlawalert:gunshotInProgress', function(targetCoords)
	...
	if Vdist(pos.x, pos.y, pos.z, targetCoords.x, targetCoords.y, targetCoords.z) < Config.BlipGunAlertRadius and Config.GunshotAlert then
		local alpha = 250
...
Citizen.CreateThread(function()
	while true do
		...
		elseif IsPedArmed(playerPed,4) and IsPedShooting(playerPed) and not IsPedCurrentWeaponSilenced(playerPed) and not (GetSelectedPedWeapon(playerPed) == 911657153) and not (GetSelectedPedWeapon(playerPed) == -1466123874) and Config.GunshotAlert then
			-- isPedArmed https://runtime.fivem.net/doc/natives/?_0x475768A975D5AD17 and not GetSelectedPedWeapon(playerPed, 911657153)
