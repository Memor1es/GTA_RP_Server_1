                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            exports["mythic_notify"]:SendAlert("error", "你沒有 ID 卡.")
                                        end
                                        end, "id_card")
                                    end
                                end
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "你沒有鋁熱炸藥.")
                                            end
                                        end, "thermal_charge")
                                    end
                                end
                                        ...
                                        if dst <= 1 and IsControlJustReleased(0, 38) then
                                            ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    self:Lockpick()
                                                elseif not output then
                                                    exports['mythic_notify']:SendAlert("error", "你沒有解鎖工具.")
                                                end
                                            end, "lockpick")
                                        end
                                    end
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    exports["mythic_notify"]:SendAlert("success", "打開我的電腦，然後開啟 HackConnect.exe")
...
function UTK:IdCard()
    ...
    --local panel = ESX.Game.GetClosestObject(pedco,{[GetHashKey("hei_prop_hei_securitypanel")]=true})
    --local panel, panelDst = ESX.Game.GetClosestEntity(ESX.Game.GetObjects(), pedco, nil, {[GetHashKey("hei_prop_hei_securitypanel")]=true})
...
function ShowVaultTimer() SetTextFont(1) SetTextProportional(0) SetTextScale(0.42, 0.42) SetTextDropShadow(0, 0, 0, 0,255) SetTextEdge(1, 0, 0, 0, 255) SetTextEntry("STRING") AddTextComponentString("~r~"..UTK.vaulttime.."~w~") DrawText(0.682, 0.96) end
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                if program == 82 and not Hacking then
                    ...
                    PopScaleformMovieFunctionVoid()
                    Hacking = true
                    exports["mythic_notify"]:SendAlert("success", "找到 IP 位置...")
                elseif program == 83 and not Hacking and Ipfinished then
                    ...
                    Hacking = true
                    exports["mythic_notify"]:SendAlert("success", "找到密碼...")
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    exports["mythic_notify"]:SendAlert("success", "執行 BruteForce.exe (暴力破解程式)")
...
AddEventHandler("utk_oh:vault", function(method)
    ...
    --local obj, objDst = ESX.Game.GetClosestEntity(ESX.Game.GetObjects(), vector3(UTK.vault.x, UTK.vault.y, UTK.vault.z), nil, {[GetHashKey(UTK.vault.type)]=true})
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports['mythic_notify']:SendAlert("error", "直到有毒氣進入前，您有90秒鐘的時間.")
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
        ...
        exports['mythic_notify']:SendAlert("error", "直到有毒氣進入前，您有60秒鐘的時間.")
        UTK.starttimer = true
        UTK.vaulttime = 60
        Citizen.CreateThread(function()
            while true do
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        exports["mythic_notify"]:SendAlert("error", "保險櫃大門會在60秒內自動關閉!")
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                exports["mythic_notify"]:SendAlert("error", "保險大門關閉!")
                ...
                if UTK.grabber then
                    ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
                        text = "$"..ESX.Math.GroupDigits(result)
                        exports["mythic_notify"]:SendAlert("success", "你偷了 "..text)
                    end)
                end
                return
            end
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job.name == "police" then
        if toggle == 1 then
            exports["mythic_notify"]:SendAlert("infrom", "太平洋警報系統已被觸發!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
