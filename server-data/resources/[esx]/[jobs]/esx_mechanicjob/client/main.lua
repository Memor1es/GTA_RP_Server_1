...
function OpenPharmacyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pharmacy', {
		...
		TriggerServerEvent('esx_mechanicjob:giveItem', data.current.item, data.current.value)
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 172.0, function(vehicle)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
					...
					if Config.MaxInService == -1 then
						ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 176.0, function(vehicle)
							local playerPed = PlayerPedId()
							TriggerEvent("disc-hotwire:forceTurnOver",vehicle)
							TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
						end)
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						--TriggerServerEvent('gcPhone:sendMessage','mechanic',ESX.GetPlayerData().name .. "賺了".. amount)
						ESX.TriggerServerCallback('esx_mechanicjob:getName', function(name)
							TriggerServerEvent('gcPhone:sendMessage','mechanic',name .. "賺了".. amount)
						end)
					end
				end
			end, function(data, menu)
...
AddEventHandler('esx_mechanicjob:onCarokit', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			...
			--		TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
			--	end
			--	ESX.ShowNotification(_U('body-_repaired'))
			-- end)
...
AddEventHandler('esx_mechanicjob:onFixkit', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		local vehicle
		if IsPedInAnyVehicle(playerPed, false) then
			ESX.ShowNotification("~r~你不能在車上修車！")
			return
		else
			vehicle = GetClosestVehicle(coords.x, coords.y, coords.z, 5.0, 0, 71)
		end

		if GetVehicleEngineHealth(vehicle) == 1000.00 then
			ESX.ShowNotification("~g~載具完好無損，不需要修復！")
		elseif GetVehicleEngineHealth(vehicle) >= 700.00 then
			ESX.ShowNotification("~w~載具狀況需要至修車廠修復！")
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			for k,v in pairs(Config.Zones) do
				if k == "Garage" then
					...
					BeginTextCommandSetBlipName('STRING')
					AddTextComponentSubstringPlayerName("採集區")
					EndTextCommandSetBlipName(blip)
				end
			end
		end
...
Citizen.CreateThread(function()
	while true do
		...
		--167 F6  168  F7
		if IsControlJustReleased(0, 168) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
