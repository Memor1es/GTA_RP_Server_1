...
RegisterNetEvent('esx_mechanicjob:giveItem')
AddEventHandler('esx_mechanicjob:giveItem', function(itemName, amount)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name ~= 'mechanic' then
		print(('[esx_mechanicjob] [^2INFO^7] "%s" attempted to spawn in an item!'):format(xPlayer.identifier))
		return
	end
					...
					TriggerEvent('esx_society:getSociety', xPlayer.job.name, function (society)
						if society ~= nil then -- verified society
							TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function (account)
								if account.money >= Config.MakeFee then -- does the society money to pay its employees?
									--xPlayer.addAccountMoney('bank', salary)
									account.removeMoney(Config.MakeFee)
									TriggerClientEvent('esx:showNotification', source, "已從公司扣款 ~r~ $".. Config.MakeFee)
									xPlayer.addInventoryItem('fixtool', 1)
									Harvest2(source)
									--TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), _U('received_paycheck'), _U('received_salary', salary), 'CHAR_BANK_MAZE', 9)
								else
									--TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), '', _U('company_nomoney'), 'CHAR_BANK_MAZE', 1)
									TriggerClientEvent('esx:showNotification', source, "公司公款不足")
								end
							end)
						else -- not a society
							--xPlayer.addAccountMoney('bank', salary)
							TriggerClientEvent('esx:showNotification', source, "你沒有公司")
							--TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), _U('received_paycheck'), _U('received_salary', salary), 'CHAR_BANK_MAZE', 9)
						end
					end)
				else
...
RegisterServerEvent('esx_mechanicjob:useFixKit')
AddEventHandler('esx_mechanicjob:useFixKit', function()
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_mechanicjob:getName', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
