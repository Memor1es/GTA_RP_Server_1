...
function OpenArmoryMenu(station)
	local elements = {
		...
		{label = _U('pharmacy_menu_title'), value = 'pharmacy'}
	}
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		elements = {
			...
			{label = _U('criminalrecord'),	value = 'criminalrecord'},
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = _U('license'), value = 'license'},
				{label = _U('search'), value = 'body_search'},
				{label = _U('handcuff'), value = 'handcuff'},
				{label = _U('unhandcuff'), value = 'unhandcuff'},
				{label = _U('seatbelt'), value = 'seatbelt'},
				{label = _U('unseatbelt'), value = 'unseatbelt'},
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'seatbelt' then
						...
						for i = 1, #players, 1 do
							TriggerServerEvent('esx_policejob:isHandcuff', GetPlayerServerId(players[i]), true)
						end
					elseif action == 'unseatbelt' then
						local playerPed = PlayerPedId()
						local players, nearbyPlayer = ESX.Game.GetPlayersInArea(GetEntityCoords(playerPed), 3.0)
						for i = 1, #players, 1 do
							TriggerServerEvent('esx_policejob:isHandcuff', GetPlayerServerId(players[i]), false)
						end
		...
		elseif data.current.value == 'criminalrecord' then
			TriggerEvent('jsfour-criminalrecord:open')
...
function OpenFineMenu(player)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine', {
		...
		elements = {
			...
			{label = _U('major_offense'),   value = 2}
	}}, function(data, menu)
	...
	ESX.TriggerServerCallback('esx_policejob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				...
				--TriggerServerEvent('gcPhone:sendMessage','police',PlayerData.name .. "賺了".. amount)
				ESX.TriggerServerCallback('esx_policejob:getName', function(name)
					TriggerServerEvent('gcPhone:sendMessage','police',name .. "賺了".. amount)
				end)
			else
...
AddEventHandler('esx_policejob:hasEnteredMarker', function(station, part, partNum)
	...
	elseif part == 'Armory' then
		...
		CurrentActionMsg  = _U('open_armory')
		CurrentActionData = {station = station}
	elseif part == 'property1' then
		CurrentAction     = 'menu_armory'
		CurrentActionMsg  = _U('open_armory')
		CurrentActionData = {station = station}
	elseif part == 'property2' then
		CurrentAction     = 'menu_armory'
		CurrentActionMsg  = _U('open_armory')
		CurrentActionData = {station = station}
...
AddEventHandler('esx_policejob:OutVehicle', function(copId)
	local playerPed = PlayerPedId()
	...
	TriggerEvent('esx_policejob:drag', copId)
end)

RegisterNetEvent('esx_policejob:Escape')
AddEventHandler('esx_policejob:Escape', function()
...
Citizen.CreateThread(function()
	while true do
		...
		if isHandcuffed then
			...
			DisableControlAction(0, Keys["K"], true) -- K
			...
			--DisableControlAction(0,Keys[Config.LeaveKey],false)
			--DisableControlAction(0,38,false)

			ESX.ShowHelpNotification('按下 ~INPUT_VEH_HEADLIGHT~ 來 ~r~掙脫手銬')
			if IsControlJustReleased(0, Keys[Config.LeaveKey])then
				--print("press")
				TriggerServerEvent('esx_policejob:CheckItem','bobbypin')
			end
...
Citizen.CreateThread(function()
    while (true) do
		ClearAreaOfPeds(425.1, -979.5, 30.7, 500, 1)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 168) and not isDead and PlayerData.job and PlayerData.job.name == 'police' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'police_actions') then
			if Config.MaxInService == -1 then
...
AddEventHandler('esx_policejob:handcuff', function(arrest, useItem)
    if isHandcuffed then
        TriggerServerEvent('esx_policejob:hasHandcuff', arrest)			
        return
    end
    ...
    TriggerServerEvent('esx_ruski_areszt:startAreszt', arrest)									-- Rozpoczyna Funkcje na Animacje (Cala Funkcja jest Powyzej^^^)
    ...
    if useItem then
        TriggerServerEvent('esx_policejob:removeHandcuff', arrest)
    end
end)

RegisterNetEvent('esx_policejob:cuffcheck')
AddEventHandler('esx_policejob:cuffcheck', function(useItem)
...
AddEventHandler('esx_policejob:unrestrain', function(arrest, useItem)
	if isHandcuffed == false then
		if arrest ~= nil then
			TriggerServerEvent('esx_policejob:hasUnrestrain', arrest)
		end
        return
    end
    isHandcuffed = false
    ...
    if useItem then
        TriggerServerEvent('esx_policejob:removeKey', arrest)
    end
end)



RegisterNetEvent('esx_policejob:uncuffcheck')
AddEventHandler('esx_policejob:uncuffcheck', function(useItem)
    local player, distance = ESX.Game.GetClosestPlayer()
    if distance ~= -1 and distance <= 3.0 then
...
---------For Seatbelt
RegisterNetEvent('esx_policejob:isHandcuff')
AddEventHandler('esx_policejob:isHandcuff', function(value)
	local ped = GetPlayerPed(-1)
	local car = GetVehiclePedIsIn(ped)

    if isHandcuffed and car ~= 0 then
        TriggerServerEvent('esx_policejob:setSeatbelt', value)
    end
end)
...
RegisterNetEvent('police:takeoutvehicle')
...
RegisterNetEvent('police:unseatnearest')
AddEventHandler('police:unseatnearest', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		local playerPed = PlayerPedId()
		local players, nearbyPlayer = ESX.Game.GetPlayersInArea(GetEntityCoords(playerPed), 3.0)
...
RegisterNetEvent('cuffs:checkinventory')
AddEventHandler('cuffs:checkinventory', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		TriggerServerEvent('esx_policejob:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
		OpenBodySearchMenu(closestPlayer)
...
RegisterNetEvent('police:getid')
AddEventHandler('police:getid', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		OpenIdentityCardMenu(closestPlayer)
	else
...
RegisterNetEvent("police:seatnearest")
AddEventHandler("police:seatnearest", function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		local playerPed = PlayerPedId()
		local players, nearbyPlayer = ESX.Game.GetPlayersInArea(GetEntityCoords(playerPed), 3.0)
