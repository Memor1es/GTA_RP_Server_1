...
AddEventHandler('crossbite_hunter:pickedUp', function()
	...
	local lucky = math.random(1,Config.OilLuck)
	local oil = xPlayer.getInventoryItem('oil')
	if lucky == 1 then
		if oil.limit ~= -1 and (oil.count + 1) > oil.limit then
			...
			xPlayer.addInventoryItem(oil.name, 1)
		end
	end
